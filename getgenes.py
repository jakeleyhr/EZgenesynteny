...
def search_gene_info(species, gene_name):
    ...
    nameswidth = len(f" synteny: {species} {gene_name} ")
    leftindent = ((terminalwidth-nameswidth)//2)
    print("▒"*terminalwidth+
          "▒"*leftindent+ 
          f" synteny: {species} {gene_name} "+ 
...
def getgenesCLI(species, gene_name, upgenes, downgenes, outputfilename, csvfilename):
    ...
    if outputfilename:
        ...
        if last_period_index != -1:  # Check if a period was found
            ...
            print(imageformat)  # Output: string
        else:
            imageformat = 'pdf'
...
def main():
    ...
    parser.add_argument("-down", "--downgenes", type = int, required=True, help="Number of upstream genes to search for")
    ...
    parser.add_argument("-f", "--input_file", default=None, help="Path to a text file containing the list of species")
